...
export default function serializeNormalizePolymorphicTypeWithinHash(hash) {
  ...
  for (let attr in hash) {
    if (
      typeof attr === 'string' &&
      attr.includes('_type') &&
      encodeURI(hash[attr]).includes('%5C')
    ) {
      ...
      if (
        hash[polymorphicRelationType] &&
        !isBlank(hash[polymorphicRelationType])
      ) {
        if (hash[polymorphicRelationType].type) {
          set(
            hash,
            `${polymorphicRelationType}._type`,
            hash[polymorphicRelationType].type
          );
        }

...
export default class ThemeService extends Service {
  ...
  get routeClassName() {
    return dasherize(
      typeof this.router.currentRouteName === 'string'
        ? this.router.currentRouteName.replace(/\./g, ' ')
        : 'fleetbase-console'
    );
  }
  ...
  get currentRouteBodyClasses() {
    if (
      this.currentRoute &&
      this.currentRoute.bodyClassNames &&
      isArray(this.currentRoute.bodyClassNames)
    ) {
  ...
  initialize(options = {}) {
    ...
    this.setEnvironment();
    this.resetScroll();
    this.setRoutebodyClassNames(
      options.bodyClassNames && isArray(options.bodyClassNames)
        ? options.bodyClassNames
        : []
    );
  ...
  setRoutebodyClassNames(classes = []) {
    document.body.classList.add(
      ...[this.routeClassName, `${this.currentTheme}-theme`, ...classes]
    );
  }

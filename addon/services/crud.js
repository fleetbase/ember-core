...
export default class CrudService extends Service {
  ...
  @action delete(model, options = {}) {
    ...
    this.modalsManager.confirm({
      title: `Are you sure to delete this ${
        options.modelName || humanize(modelName).toLowerCase()
      }?`,
      args: ['model'],
      model,
      confirm: (modal) => {
        ...
        return model.destroyRecord().then((model) => {
          this.notifications.success(
            options.successNotification ||
              `'${
                options.modelName || model.name || humanize(modelName)
              }' has been deleted.`
          );
        });
      },
  ...
  @action bulkAction(verb, selected, options = {}) {
    ...
    const actionMethod = (
      typeof options.actionMethod === 'string' ? options.actionMethod : `POST`
    ).toLowerCase();
        ...
        return this.fetch
          ...
          .catch((error) => {
            ...
            this.notifications.serverError(
              error,
              'Unable to download API credentials export.'
            );
          });
      },
      ...options,
